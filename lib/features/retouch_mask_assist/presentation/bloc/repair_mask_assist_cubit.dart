...
part 'repair_mask_assist_state.dart';

class RepairMaskAssistCubit extends Cubit<RepairMaskAssistState> {
  RepairMaskAssistCubit({
    ...
    required ExportProcessingMaskUseCase exportProcessingMaskUseCase,
        ...
        super(const RepairMaskAssistState());
  ...
  Future<void> setImage(Uint8List imageBytes, {bool resetMode = false}) async {
    ...
    emit(
      state.copyWith(
        ...
        clearMaskData: true,
        expansionLevel: 0,
        feather: resetMode ? 3 : state.feather,
        previewVisible: true,
        canUndo: false,
        canRedo: false,
      ),
    ...
    try {
      final result = await _generateMaskSuggestionUseCase(
        MaskSuggestionRequest(
          ...
          toolType: RetouchToolType.repair,
        ),
      );
  ...
  Future<void> retrySuggestion() async {
  ...
  Future<void> updateFeather(double feather) async {
    ...
    if (_undoStack.isNotEmpty) {
      _undoStack[_undoStack.length - 1] = _MaskSnapshot(
        Uint8List.fromList(state.maskAlpha!),
        feather,
