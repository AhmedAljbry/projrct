...
class _OperationTile extends StatelessWidget {
  const _OperationTile({required this.job, this.onTap});

  final BackgroundJob job;
  final ValueChanged<BackgroundJob>? onTap;
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      ...
      onTap: onTap == null ? null : () => onTap!(job),
      child: Container(
        ...
        child: Row(
          children: [
            ...
            if (isActive)
              IconButton(
                onPressed: () =>
                    context.read<JobQueueCubit>().cancelJob(job.jobId),
  ...
  String _statusText(BuildContext context, BackgroundJob job) {
    ...
    final shouldShowStoredMessage = switch (job.status) {
      JobStatus.failed || JobStatus.cancelled || JobStatus.retryWaiting => true,
      _ => false,
    };
    if (shouldShowStoredMessage && message != null && message.isNotEmpty) {
      return message;
    }
